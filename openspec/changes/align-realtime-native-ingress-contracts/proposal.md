...
- 将 QMT 正式模式定义为 `QMT_REALTIME_MODE=builtin|off`，生产 desired state 默认为 `builtin`，`off` 仅作为受控回滚；实际订阅仍由正式 realtime allowlist 控制。
...
- `mist-production-baseline`: 将 QMT 生产 desired state 从默认 `off` 调整为正式 `builtin`，保留显式 `off` 回滚。
