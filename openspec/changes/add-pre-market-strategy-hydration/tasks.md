...
## Phase 2: Polars 向量质检与内存滑窗预热 (`SharedStrategyWindowStore`)

- [ ] 2.1 实现 Polars 向量化数据质检函数 `validateBarSeriesWithPolars`（检查时间单调递增、非负价格、High >= Low、零 NaN/Null 缺失）
- [ ] 2.2 在 `libs/signal/src/runtime/shared-strategy-window.store.ts` 中实现 `warmup(marketData, targets, anchorAt)` 方法
- [ ] 2.3 确保 `warmup` 集成容量复用检查（已有且容量满足则幂等跳过）、Polars 质检拦截与静态指标分析缓存挂载
- [ ] 2.4 确保单标的异常（拉取错误、Polars 质检不通过）被捕获隔离，返回结构化 `WindowWarmupReport`
- [ ] 2.5 编写 `shared-strategy-window.store.spec.ts` 针对 Polars 质检与 `warmup` 的全量单元测试（质检通过/拒绝、容量复用、异常隔离）

## Phase 3: 信号运行时编排与双重触发 (`apps/signal`)

- [ ] 3.1 在 `SignalRealtimeStartupService` 中集成事件驱动的异步预热逻辑（注册表初始化与变更后触发）
- [ ] 3.2 增加 09:20 交易日定时调度器，在交易日开盘前 09:20 执行全量活跃策略滑窗预热与质检
- [ ] 3.3 补充 `SignalRealtimeStartupService.spec.ts` 针对预热调度与触发的测试用例

## Phase 4: 可观测性与健康诊断 (`apps/signal`)

- [ ] 4.1 在 `SignalHealthVo` 及 diagnostics 中增加预热状态信息与 Polars 质检统计
