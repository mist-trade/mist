# Mist Production Baseline Rerun Evidence - 2026-07-03
...
| --- | --- | --- |
| `mist` | `9f1d239950bc94304887dbb49ae7f38042a73e13` | Backend image built, deployed, and smoke-tested |
...
| Repository | Ref | Evidence |
| --- | --- | --- |
| `mist-datasource` | `7bb2ac7069c2e9fe36e8d9bfbe1d96cd951c87e5` | Local workspace ref only; this rerun smoke-tested the already deployed host WinSW service in place and did not reinstall or re-checkout datasource code |
...
| Component | Image |
| --- | --- |
| Backend | `ghcr.io/mist-trade/mist:9f1d239950bc94304887dbb49ae7f38042a73e13` |
...
| --- | --- | --- | --- |
| `mist` | `Build Docker Images` | `28605798208` | Success for `9f1d239950bc94304887dbb49ae7f38042a73e13` |
...
- Backend: `5460d9a2861d55d9597ea88731134542de77c3c0`
...
| --- | --- |
| Run | `28622940934` |
| URL | `https://github.com/mist-trade/mist-deploy/actions/runs/28622940934` |
| Job | `84882836189` |
| Final status | Success |
| `mist-deploy` ref | `61e631e095f88bc7a51092b6ea909b2da868a981` |
| Backend image | `ghcr.io/mist-trade/mist:9f1d239950bc94304887dbb49ae7f38042a73e13` |
...
- Backend image `9f1d239950bc94304887dbb49ae7f38042a73e13` was pulled.
...
| --- | --- |
| Run | `28623034896` |
| URL | `https://github.com/mist-trade/mist-deploy/actions/runs/28623034896` |
| Job | `84883132544` |
| Final status | Success |
...
| --- | --- |
| Run | `28623115601` |
| URL | `https://github.com/mist-trade/mist-deploy/actions/runs/28623115601` |
| Job | `84883392320` |
| Final status | Success |
...
Pre-recovery live quote verification:
...
5. The observer received a `quote` event with snapshot fields including
   ...
   `Amount=612236.06`, `Provider="tdx"`, and `AsOf="<timestamp>"`.
...
7. Post-cleanup datasource health returned HTTP `200` with
   ...
   `quoteCallbackCount=26`, `quoteCallbackRejectedCount=0`,
...
This rerun also exercised the operator-requested TDX terminal recovery path.
This is the recovery flow for TDX logout, stale or duplicate strategy identity,
or stuck TQ initialization; it is separate from ordinary datasource service
restart.
...
| --- | --- |
| Run | `28626516956` |
| URL | `https://github.com/mist-trade/mist-deploy/actions/runs/28626516956` |
| Job | `84894122923` |
| Final status | Success |
...
Post-recovery live quote verification:

1. `POST http://<gateway-hostname>/api/mist/v1/collector/test/tdx-streaming/subscribe`
   with `{"code":"600519","period":1,"testOnly":true}` returned HTTP `200`,
   `success=true`, `count=1`, request id `http-<request-id>`, and timestamp
   `2026-07-02T22:53:14Z`.
2. A Mac-side read-only WebSocket observer connected to
   `ws://<windows-lan-ip>:9001/ws/quote/<observer-client-id>`.
3. The observer received a ready message showing
   `leaderClientId="mist-backend-tdx"` and `active=["600519.SH"]`.
...
7. Post-cleanup datasource health returned HTTP `200` with
   `subscribedCount=0`, `activeSubscriptions=[]`,
   `quoteCallbackCount=3`, `quoteCallbackRejectedCount=0`,
   `lastQuoteCallbackSymbol="600519.SH"`,
   `lastQuoteCallbackAccepted=true`, `eventQueueDepth=0`, and
   `collectorState="running"`.

Conclusion: TDX recovery restarted the terminal and datasource service
successfully, and the backend-leader live quote path still worked after
recovery cleanup.
...
restore rehearsal, datasource runtime smoke, TDX terminal recovery,
...
- TDX terminal recovery passed with TDX stop/start, runtime login, datasource
  restart, datasource smoke, and a post-recovery live quote check.
