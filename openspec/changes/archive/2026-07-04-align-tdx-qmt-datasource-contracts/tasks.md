Archive note (2026-07-04): the last live-smoke item is closed by the production
baseline evidence. Windows runtime smoke run `28655501458` passed against the
logged-in TDX datasource after deployment, and the backend leader-path live
quote verification was captured separately in the same baseline evidence.
...
- [x] 9.9 Covered by Windows runtime smoke run `28655501458` against the
      logged-in TDX terminal after deployment, with leader-path live quote
      verification captured separately in the production baseline evidence.
