...
@Injectable()
export class QmtCollectionStrategy implements IDataCollectionStrategy {
  readonly source = DataSource.QMT;
  readonly mode: CollectionMode = 'polling';
  private readonly logger = new Logger(QmtCollectionStrategy.name);
  ...
  async start(): Promise<void> {
    this.logger.log('Starting QMT collection strategy');
  }

  async stop(): Promise<void> {
    this.logger.log('Stopping QMT collection strategy');
  }
}
