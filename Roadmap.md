...
该项目迭代肯定要花费大量时间，目前一期的目标是上证指数的分析，同时数据采集部分通过 mist-datasource 完成
所需的模块如下
...
后续策略分析先走确定性后端模块，而不是独立模型编排服务。第一版目标是将指标和缠论结果转成结构化策略信号，再由告警模块消费。

#### 信号模型

1. 策略定义 - 固定策略名称、适用标的、周期、参数和启停状态。
2. 信号计算 - 基于已落库 K 线、MACD/KDJ/RSI、缠论笔/分型/中枢等结果生成客观信号。
3. 证据记录 - 保存触发条件、关键指标值、K 线窗口、信号强度和失效时间。
4. 回放验证 - 用历史数据统计信号出现后的收益、回撤、胜率和样本数。
5. 告警消费 - 告警模块读取已确认信号，做去重、冷却和触达。
...
切换成monorepo架构，主核心是mist，AI/机器人入口通过 MCP Server 和 mist-skills 对接。

继续拆分架构，将mist中定时器部分拆出来，目前主核心是mist，schedule为定时器脚本。分别独立部署
